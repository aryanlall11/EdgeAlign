...
from tensorflow.keras.layers import Dense, Conv2D, Input, Flatten, Reshape, LSTM, Dropout, Embedding, Bidirectional, GRU
...
class Model():
    ...
    def build_model(self, output_activation="softmax", loss="categorical_crossentropy", optimizer="adam"):
        ...
        model = Sequential()
        model.add(Input(shape = (1, window)))
        model.add(Reshape(target_shape=[window]))

        # Embedding
        model.add(Embedding(26, 4, input_length=window))
        # Conv blocks
